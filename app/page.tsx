...
confidence:number
 ...
 const predictionConfidence=result?.data?.at(0)?.confidences?.at(0)?.confidence
...
<div className="flex flex-1 w-full flex-col lg:flex-row justify-center items-center text-center  gap-x-5 xl:gap-x-0 space-y-10 ">
  ...
  { !imagePrediction && <div className="inline text-xs text-start"> * Upload image of <span className='font-extrabold'>Single</span> leaf of any of these <Plants /> only</div>}
  <ImageDropzone
     setImages={setImages}
        images={images}
        setImagePrediction={  setImagePrediction}
      />
      <Button className="w-full" size={'lg'} onClick={async()=> await getPrediction()} disabled={!images}>
        
     {  isPredictionLoading ? <div className="flex flex-row space-x-3">
<LoadingSVG /> 
Loading
     </div>: "Determine leaf condition"}
      </Button>
      {imagePrediction && <p className="text-start">{imagePrediction.includes('healthy')? `There's ${confidence}% chance this is a healthy ${imagePrediction.replace(new RegExp("\\b" + "healthy" + "\\b", "gi"), "")} leaf`: `There's ${confidence}% this leaf has ${imagePrediction}. Learn more on the disease using our chatbot`}</p>}
  </div>
      ...
      <div className="flex flex-col w-full max-w-2xl  mx-auto stretch ">
        ...
        <form onSubmit={handleSubmit} ref={formRef} className="relative ">
          <Textarea
            ...
            value={input}
            placeholder={imagePrediction?.includes('healthy') || imagePrediction===undefined? "Ask me about any plant disease...": `Ask me about ${imagePrediction}`}
